...
for i in range(1,4):
    aster=Asteroid(img_asteroid,randint(30,win_height-30),-40,80,50,randint(1,3))
    asteroids.add(aster)
...
while run:
    for e in event.get():
        if e.type==QUIT:
            ...
            if e.key==K_SPACE:
                if is_rel==False and count_bullets<=4:
                    count_bullets+=1
                    fire_sound.play()
                    ship.fire()
                elif count_bullets>4 and is_rel==False:
                    is_rel=True
                    last_time=timer()
    if not finish:
        ...
        if sprite.spritecollide(ship,asteroids,True):
            hp-=1
            aster=Asteroid(img_asteroid,randint(80,win_height-80),-40,80,50,randint(1,3))
            asteroids.add(aster)
